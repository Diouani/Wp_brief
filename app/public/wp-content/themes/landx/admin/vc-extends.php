...
if ( !function_exists( 'landx_parse_text' ) ):
    function landx_parse_text( $text, $args = array( ) ) {
        if ( is_array( $args ) ) {
...
add_filter( 'vc_iconpicker-type-pixden', 'landx_iconpicker_type_pixden' );

function landx_iconpicker_type_pixden( $icons ){
...
add_filter( 'vc_iconpicker-type-perch', 'landx_iconpicker_type_perch' );

function landx_iconpicker_type_perch( $icons ){
  $pixden = array( 
...
function landx_vc_icontype_dropdown( $name = 'icon_type', $value = array(
                'Linecons' => 'linecons',
                'Entypo' => 'entypo',
                'Typicons' => 'typicons',
...
function landx_vc_icon_set( $type, $name = 'icon_pixden', $value= 'pe-7s-diamond', $dependency = '' ){
 
  $arr = array(
          'type' => 'iconpicker',
          'heading' => __( 'Icon', 'landx' ),
          'param_name' => $name,
          'value' => $value,
          'settings' => array(
              'emptyIcon' => true,
          ...
          'description' => __( 'Select icon from library.', 'landx' ),
...
if( !function_exists('landx_get_posts_dropdown') ):
function landx_get_posts_dropdown( $args = array() ) {
    global $wpdb, $post;
...
if( !function_exists('landx_get_terms') ):
function landx_get_terms( $tax = 'category', $key = 'id' ) {
    $terms = array();
...
function landx_number_settings_field( $settings, $value ) {
...
function landx_vc_image_upload_settings_field($settings, $value){
  return '<div class="landx-upload-container">
      ...
      <a href="javascript:;" class="button landx-upload-button"><span class="wp-media-buttons-icon"></span>'.__( 'Media manager', 'landx' ).'</a>
      <img width="80" src="'.esc_url($value).'" alt="">     
    </div>';
}

function landx_perch_select_settings_field( $args, $value ) {
...
if(function_exists('vc_add_shortcode_param')){
  ...
  vc_add_shortcode_param( 'number', 'landx_number_settings_field' );
  vc_add_shortcode_param( 'perch_select', 'landx_perch_select_settings_field' );
  vc_add_shortcode_param( 'image_upload', 'landx_vc_image_upload_settings_field' );
...
foreach (glob(LANDX_DIR."/admin/vc-extends/*.php") as $filename){   
    include $filename;
}
endif;
...
* @return array
*/
function landx_vc_get_params_value($args = array(), $_content = NULL){
    $array = array();
...
require_once LANDX_DIR . '/admin/v2/inc.php';