...
class AjaxCart {
  constructor() {
    ...
    jQuery(window).on("resize", () => {
      if (zota_settings.mobile && $(window).width() < 992) {
  ...
  _initAjaxPopupContent(button) {
    let cart_modal = $('#tbay-cart-modal'),
        ...
        cart_notification = zota_settings.popup_cart_noti,
        string = '';
...
class ProductItem {
  initAddButtonQuantity() {
    ...
    input.each(function () {
      ...
      $(`<button class="minus" type="button" value="&nbsp;">${zota_settings.quantity_minus}</button>`).insertBefore($(this));
      $(`<button class="plus" type="button" value="&nbsp;">${zota_settings.quantity_plus}</button>`).insertAfter($(this));
    });
  }
...
class SingleProduct {
  ...
  _initFeatureVideo() {
    if (typeof zota_settings === "undefined") return;
    let featured = $(document).find(zota_settings.img_class_container + '.tbay_featured_content');
    if (featured.length === 0) return;
    let featured_index = featured.index(),
        featured_gallery_thumbnail = $(zota_settings.thumbnail_gallery_class_element).get(featured_index);
...
jQuery(window).on('elementor/frontend/init', function () {
  if (typeof zota_settings !== "undefined" && jQuery.isArray(zota_settings.elements_ready.products)) {
