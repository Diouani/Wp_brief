...
if ( ! function_exists( 'zota_custom_share_code' ) ) {
	function zota_custom_share_code( $title, $link, $media ) {
		if( !zota_tbay_get_config('enable_code_share', true) ) return;
...
if ( ! function_exists( 'zota_tbay_post_share_box' ) ) {
  	function zota_tbay_post_share_box() {
  		if( !zota_tbay_get_config('enable_code_share',false) || !zota_tbay_get_config('show_blog_social_share', false) ) return;
	...
	function zota_tbay_render_breadcrumbs() {
		...
		if ( is_page() && is_object($post) ) { 
			...
			if ( isset($show) && $show != 'yes' ) {
				if( !zota_tbay_is_home_page() && !empty($title_bottom) )  echo '<div class="title-not-breadcrumbs"><div class="container">'. trim( $title_bottom ) . '</div></div>'; 
				return '';
			}
		...
		if ($breadcrumbs_layout !== 'image' && !zota_tbay_is_home_page() && !empty($title_bottom) ) echo '<div class="title-not-breadcrumbs"><div class="container">'. trim( $title_bottom ) . '</div></div>';
...
if ( ! function_exists( 'zota_tbay_paging_nav' ) ) {
	function zota_tbay_paging_nav() {
		...
		<nav class="navigation paging-navigation">
			<h1 class="screen-reader-text hidden"><?php esc_html_e( 'Posts navigation', 'zota' ); ?></h1>
	...
	function zota_tbay_post_nav() {
		...
		<nav class="navigation post-navigation">
			...
			<div class="nav-links clearfix">
				<?php
				if ( is_attachment() ) :
					previous_post_link( '%link','<div class="col-lg-6"><span class="meta-nav">'. esc_html__('Published In', 'zota').'</span></div>');
				else :
...
if (!function_exists('zota_get_display_header_builder') ) {
	function zota_get_display_header_builder() {
		...
		foreach ( $posts as $post ) {
			if( zota_elementor_is_activated() && Elementor\Plugin::$instance->db->is_built_with_elementor( $post->ID ) ) {
				return Elementor\Plugin::instance()->frontend->get_builder_content_for_display( $post->ID, zota_get_elementor_css_print_method() );
			} else {
...
if (!function_exists('zota_get_display_footer_builder') ) {
	function zota_get_display_footer_builder($footer) {
		$args = array(
			'name'        => $footer,
...
if (!function_exists('zota_tbay_display_footer_builder') ) {
	function zota_tbay_display_footer_builder($footer) {
		echo zota_get_display_footer_builder($footer);
	}
}
...
if ( ! function_exists( 'zota_get_social_html' ) ) {
	function zota_get_social_html($key, $value, $title, $link, $media) {
		...
		switch ($key) {
			...
			case 'twitter':
				$output = sprintf(
					'<a class="share-twitter zota-twitter" href="http://twitter.com/share?text=%s&url=%s" title="%s" target="_blank"><i class="fab fa-twitter"></i></a>',
					esc_attr( $title ),
					urlencode( $link ),
					urlencode( $title )
				);
