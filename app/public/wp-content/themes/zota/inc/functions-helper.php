...
if ( !function_exists('zota_tbay_get_header_layouts') ) {
	function zota_tbay_get_header_layouts() {
		$headers = array( 'header_default' => esc_html__('Default', 'zota'));
		$args = array(
			'posts_per_page'   => -1,
			'offset'           => 0,
			'orderby'          => 'date',
...
if ( !function_exists( 'zota_tbay_autocomplete_search' ) ) { 
    function zota_tbay_autocomplete_search() {
        if ( zota_tbay_get_global_config('autocomplete_search', true) ) {
...
add_action( 'init', 'zota_tbay_autocomplete_search' );
...
if ( !function_exists('zota_tbay_get_custom_tab_layouts') ) {
  function zota_tbay_get_custom_tab_layouts() {
    ...
    if( !zota_is_Woocommerce_activated() ) return $tabs;
    $args = array(
      'posts_per_page'   => -1,
      'offset'           => 0,
      'orderby'          => 'date',
