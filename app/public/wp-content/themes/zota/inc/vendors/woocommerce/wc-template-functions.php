...
if ( !function_exists('zota_tbay_woocommerce_get_cookie_display_mode') ) {
    function zota_tbay_woocommerce_get_cookie_display_mode() {
        ...
        if( isset($_COOKIE['zota_display_mode']) && $_COOKIE['zota_display_mode'] == 'grid' ) {
            $woo_mode = 'grid';
        } else if ( isset($_COOKIE['zota_display_mode']) && $_COOKIE['zota_display_mode'] == 'grid2' ) {
            $woo_mode = 'grid2';
        } else if( isset($_COOKIE['zota_display_mode']) && $_COOKIE['zota_display_mode'] == 'list' ) {
            $woo_mode = 'list';
        }
...
if( ! function_exists( 'zota_swatches_list' ) ) {
    function zota_swatches_list( $attribute_name = false ) {
        ...
        if( ! $attribute_name ) {
            $attribute_name = zota_get_swatches_attribute();
        }
...
if ( !function_exists('zota_tbay_get_woocommerce_layout_configs') ) {
    function zota_tbay_get_woocommerce_layout_configs() {
        ...
        if ( !is_singular( 'product' ) ) {
            ...
            if( zota_woo_is_wcmp_vendor_store() ) {
    ...
    function zota_the_yith_compare($product_id) {
        ...
        if( class_exists( 'YITH_Woocompare' ) ) { ?>
            ...
            <div class="yith-compare">
                <a href="<?php echo esc_url( wp_nonce_url( add_query_arg( $url_args ), $action_add ) ); ?>" title="<?php esc_attr_e('Compare', 'zota'); ?>" class="compare" data-product_id="<?php echo esc_attr($product_id); ?>">
                    <span><?php esc_html_e('Compare', 'zota'); ?></span>
                </a>
            </div>
        <?php } 
...
if ( ! function_exists( 'zota_woo_is_wcmp_vendor_store' ) ) {
    function zota_woo_is_wcmp_vendor_store() {
...
if ( ! function_exists( 'zota_ajax_product_remove' ) ) {
    function zota_ajax_product_remove(){  
        // Get mini cart
        ob_start();  
    ...
    add_action( 'wp_ajax_product_remove', 'zota_ajax_product_remove' );
    add_action( 'wp_ajax_nopriv_product_remove', 'zota_ajax_product_remove' );
}
...
if ( ! function_exists( 'zota_woocommerce_quantity_mode_add_class' ) ) {
    ...
    function zota_woocommerce_quantity_mode_add_class( $classes ){
        if( !zota_woocommerce_quantity_mode_active() ) return $classes;
...
if ( ! function_exists( 'zota_variation_swatches_pro_group_button' ) ) {
    function zota_variation_swatches_pro_group_button() {
        if( !zota_is_woo_variation_swatches_pro() ) return;
...
if ( ! function_exists( 'zota_woocommerce_quantity_mode_group_button' ) ) {
    function zota_woocommerce_quantity_mode_group_button() {
        if( !zota_woocommerce_quantity_mode_active() || zota_is_woo_variation_swatches_pro() ) return;
...
if ( ! function_exists( 'zota_woocommerce_cart_item_name' ) ) {
    function zota_woocommerce_cart_item_name( $name, $cart_item, $cart_item_key ) { 
        if( !zota_tbay_get_config('show_checkout_image', false) || !is_checkout() )  return $name;
