...
if( ! function_exists( 'zota_tbay_regiter_vendor_dokan_popup' ) ) {
    function zota_tbay_regiter_vendor_dokan_popup() {
        $outputs = '<div class="vendor-register">';
        $outputs .= sprintf( __( 'Are you a vendor? <a href="%s">Register here.</a>', 'zota' ), get_permalink( get_option('woocommerce_myaccount_page_id') ) );
        $outputs .= '</div>';
        echo trim($outputs);
    }
    add_action( 'zota_custom_woocommerce_register_form_end', 'zota_tbay_regiter_vendor_dokan_popup', 5 );
}
...
if(!function_exists('zota_dokan_get_number_of_products_of_vendor')){
    function zota_dokan_get_number_of_products_of_vendor() {
        ...
        if( !zota_woo_is_vendor_page() ) return;
...
if ( ! function_exists( 'zota_dokan_description' ) ) {
    function zota_dokan_description( $description ) {
        if( !zota_woo_is_vendor_page() ) return $description;
