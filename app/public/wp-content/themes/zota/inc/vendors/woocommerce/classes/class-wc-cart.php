...
if ( ! class_exists( 'Zota_Cart' ) ) :
	...
	class Zota_Cart  {
		...
		public function woocommerce_cart_position() {
	        ...
	        $position = zota_tbay_get_config('woo_mini_cart_position', 'popup');

	        $position = ( isset($_GET['ajax_cart']) ) ? $_GET['ajax_cart'] : $position;

	        $position =  (!in_array($position, $position_array)) ? zota_tbay_get_config('woo_mini_cart_position', 'popup') : $position;
		...
		public function add_to_cart_message_html_mobile( $message ) {
			if ( isset( $_REQUEST['zota_buy_now'] ) && $_REQUEST['zota_buy_now'] == true ) {
	            return __return_empty_string();
	        }

	        if ( wp_is_mobile() && ! intval( zota_tbay_get_config('enable_buy_now', false) ) ) {
	            return __return_empty_string();     
	        } else {
	            return $message;
	        }
		...
		public function show_cart_mobile() {
			$active = zota_tbay_get_config('enable_add_cart_mobile', false);
