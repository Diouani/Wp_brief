...
if( ! function_exists( 'zota_options_woocom_save_proddata_custom_fields' ) ) {
  ...
  function zota_options_woocom_save_proddata_custom_fields($product) {
			$video_url = isset( $_POST['_zota_video_url'] ) ? $_POST['_zota_video_url'] : '';
      $old_value_url = $product->get_meta( '_zota_video_url' );

			if( $video_url !== $old_value_url ) {
				$product->update_meta_data( '_zota_video_url', $video_url );
				$img_id = '';
				if ( ! empty( $video_url ) ) {
					$video_info = explode( ':', zota_video_type_by_url( $video_url ) );
					$img_id     = zota_save_video_thumbnail( array(
						'host' => $video_info[0],
						'id'   => $video_info[1] 
					) );
				}
			...
			$tab_id = isset( $_POST['_zota_custom_tab'] ) ? $_POST['_zota_custom_tab'] : '';
      $old_tab_id = $product->get_meta( '_zota_custom_tab' );
      
			$tab_priority = isset( $_POST['_zota_custom_tab_priority'] ) ? $_POST['_zota_custom_tab_priority'] : '';
			$old_tab_priority = $product->get_meta( '_zota_custom_tab_priority' );
      
      if( $tab_id !== $old_tab_id ) {
				$product->update_meta_data( '_zota_custom_tab', $tab_id );
      }
      
      if( $tab_priority !== $old_tab_priority ) {
				$product->update_meta_data( '_zota_custom_tab_priority', $tab_priority );
			}
            ...
            foreach ( $attachments as $key => $attachment_id ) {
              ...
              if( $key != 0) {
                ...
                echo '<li class="image" data-attachment_id="' . esc_attr( $attachment_id ) . '">
                  ' . $attachment . '
                  <ul class="actions">               
                    <li><a href="#" class="delete tips" data-tip="' . esc_attr__( 'Remove product image', 'zota' ) . '">' . esc_html__( 'Remove product image', 'zota' ) . '</a></li>
                  </ul>
                </li>';
