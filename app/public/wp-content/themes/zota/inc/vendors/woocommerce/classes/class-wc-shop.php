...
if ( ! class_exists( 'Zota_Shop_WooCommerce' ) ) :
	...
	class Zota_Shop_WooCommerce  {
		...
		public function __construct() {
			...
			add_action( 'zota_woo_before_shop_loop_item_caption', array( $this, 'grid_variable_swatches_pro'), 10 );
			add_action( 'zota_woo_list_after_short_description', array( $this, 'list_variable_swatches_pro'), 5 );
		...
		public function shop_display_modes() {
			...
			$active  = apply_filters( 'zota_woo_config_display_mode', 10,2 );
			
	        if ( !$active || !wc_get_loop_prop( 'is_paginated' ) || ( !woocommerce_products_will_display() && !zota_woo_is_vendor_page() ) ) {
	            return;
	        }
		...
		public function shop_per_page() {
			...
			if( isset($_GET['product_per_page']) && is_numeric($_GET['product_per_page']) ) {
	            ...
	            $value = zota_tbay_get_config('number_products_per_page', 12);          
	        }
		...
		public function grid_variable_swatches_pro() {
			...
			if ( zota_is_woo_variation_swatches_pro()  ) {
			    add_action( 'zota_woo_after_shop_loop_item_caption', 'wvs_pro_archive_variation_template', 10 ); 
			}
		...
		public function list_variable_swatches_pro() {
			if ( zota_is_woo_variation_swatches_pro() ) {
	            add_action( 'zota_woo_list_after_short_description', 'wvs_pro_archive_variation_template', 20 ); 
	        }
		}
		...
		public function shop_load_more() {
	        ...
	        if((strpos($category, ',') !== false )) {
	            ...
	            $loop = zota_tbay_get_products( $categories, $type , $paged, $number_load, '', '', $number, $offset );
	        } else {

	            if( $category == -1 ) {
	                $loop = zota_tbay_get_products( '', $type , $paged, $number_load, '', '', $number, $offset );
	            } else {
	              $loop = zota_tbay_get_products( array($category), '' , $paged, $number_load, '', '', $number, $offset );  
	            } 
