	...
	class Zota_Single_WooCommerce  {
		...
		public function __construct() {
    		...
    		if(!wp_is_mobile() ) {
			    ...
			    add_action( 'zota_sticky_menu_bar_product_summary', 'woocommerce_template_single_title', 5 );
	        ...
	        <?php if(class_exists('YITH_WCWL') || class_exists('YITH_Woocompare')){ ?>
	            ...
	            <?php if(class_exists('YITH_WCWL')) { ?> 
	                <div class="tbay-wishlist">
	                   <?php zota_the_yith_wishlist(); ?>
	                </div>  
	            <?php } ?>
	            <?php if(class_exists('YITH_Woocompare')){ ?>
	                <div class="tbay-compare">
	                    <?php zota_the_yith_compare($product->get_id()); ?>
	                </div>
	            <?php } ?>
	            </div>
	        <?php } ?>
		...
		public function buy_now_html() {
	        ...
	        echo sprintf( '<button class="'. esc_attr($class) .'">%s</button>', esc_html__('Buy Now', 'zota') );
	        echo '<input type="hidden" value="0" name="zota_buy_now" />';
		}

		public function buy_now_redirect( $url ) {

	 		if ( ! isset( $_REQUEST['zota_buy_now'] ) || $_REQUEST['zota_buy_now'] == false ) {
	            return $url; 
	        }
	        ...
	        $redirect = zota_tbay_get_config('redirect_buy_now', 'cart') ;
	                    ...
	                    foreach ( $image_post_ids as $image_post_id ) {
	                        if ( ! wc_is_valid_url( $image_post_id ) ) {
	                            ...
	                            $outputs .= '<li><a class="lightbox-gallery" href="'. $img_src_open .'"><img class="review-images"
	                                     src="' . esc_attr($img_src) .'" alt="'. esc_attr( apply_filters( 'woocommerce_photo_reviews_image_alt', $image_alt, $image_post_id, $comment ) ) .'"/></a></li>';
	                            $i++;
	                        }
	                    }
	                }
