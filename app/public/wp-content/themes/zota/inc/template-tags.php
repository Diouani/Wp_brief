...
/**
 * Flush out the transients used in {@see zota_tbay_categorized_blog()}.
 *
 * @since Zota 1.0
 */
function zota_tbay_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'zota_tbay_categories' );
}
add_action( 'edit_category', 'zota_tbay_category_transient_flusher' );
add_action( 'save_post',     'zota_tbay_category_transient_flusher' );
...
if ( ! function_exists( 'zota_search_only_title' ) ) {
	function zota_search_only_title( $search, $wp_query ) {
...
if ( ! function_exists( 'zota_search_options' ) ) {
	function zota_search_options( $search, $wp_query ) {

		if( !(defined('TBAY_ELEMENTOR_ACTIVED') && TBAY_ELEMENTOR_ACTIVED) ) return $search;
		$where = zota_search_only_title($search, $wp_query);

		return $where; 
	}
	add_filter( 'posts_search', 'zota_search_options', 20, 2 );
}
