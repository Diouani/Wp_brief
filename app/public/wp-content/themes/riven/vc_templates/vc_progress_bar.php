 ...
 */
$title = $values = $units = $bgcolor =$icon_fontawesome = $select_font = $css = $custombgcolor = $show = $customtxtcolor = $options = $el_class = '';
$output = '';
...
if( $show=="display"){
	foreach ( $graph_lines_data as $line ) {
		...
		$output .= '<small class=" icon-progress '.$line['icon_fontawesome'] . $unit .'">'.''. '</small>';
	...
	foreach ( $graph_lines_data as $line ) {
		...
		$output .= '<div class="vc_general vc_single_bar progress-bar-style2' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ?
				' vc_progress-bar-color-' . $line['color'] : '' )
			. '">';
		$output .= '<small class="vc_progress_value"' . '>' . $line['value'] .'%' . $unit . '</small>';
		$output .= '<small class="vc_label"' . $line['txtcolor'] . '>' . $line['label'] . $unit . '</small>';
