...
if ( ! class_exists( 'riven_Gallery_Metabox' ) ) :

class riven_Gallery_Metabox {
    ...
    function __construct( $metakey = null ) {
        ...
        $this->supported_post_types = array( 'post','portfolio' );
        ...
        add_action( 'admin_enqueue_scripts', array( $this, 'riven_gallery_metabox_enqueues' ) );
        add_action( 'add_meta_boxes', array( $this, 'riven_gallery_add_metabox' ) );
        add_action( 'save_post', array( $this, 'riven_gallery_metabox_save' ) );
    }

    function riven_gallery_metabox_enqueues( $screen ) {
    ...
    function riven_gallery_add_metabox( $post_type ) {
        if ( in_array( $post_type, $this->supported_post_types ) ) {
            add_meta_box(
                'gallery-metabox',
                esc_html__( 'Images Gallery', 'riven' ),
                array( $this, 'riven_gallery_metabox_fields' ),
                $post_type,
                'side',
                'core'
            );
    ...
    function riven_gallery_metabox_fields( $post ) {
        ...
        <div class="sn-gallery-wrap">
            <div class="sn-gallery-inner">
                <a href="#" class="button gallery-add-images"
                    title="<?php echo esc_html__( 'Add image(s) to gallery', 'riven' ) ?>"
                    data-uploader-title="<?php echo esc_html__( 'Add image(s)', 'riven'); ?>"
                    data-uploader-button-text="<?php echo esc_html__( 'Add image(s)', 'riven'); ?>"><?php echo esc_html__( 'Add image(s)', 'riven' ); ?></a>
                ...
                if ( $images ) :
                    ...
                    <li>
                        ...
                        <a href="#" class="change-image"
                            title="<?php echo esc_html__( 'Change image', 'riven' ); ?>"
                            data-uploader-title="<?php echo esc_html__( 'Change image', 'riven' ); ?>"
                            data-uploader-button-text="<?php echo esc_html__( 'Change image', 'riven' ); ?>"><i class="dashicons dashicons-edit"></i></a>
                        <a href="#" class="remove-image" title="<?php echo esc_html__( 'Remove Image', 'riven' ); ?>"><i class="dashicons dashicons-no"></i></a>
                    </li><?php
                    endforeach;
                endif; ?>
                </ul>
    ...
    function riven_gallery_metabox_save( $post_id ) {
...
if ( is_admin() ) {
    add_action( 'load-post.php', 'riven_gallery_init' );
    add_action( 'load-post-new.php', 'riven_gallery_init' );
}

function riven_gallery_init() {
    new riven_Gallery_Metabox();
}
