...
if ( ! class_exists( 'riven_Related_Posts' ) ) :

class riven_Related_Posts {
    ...
    function __construct(){
        ...
        add_action( 'admin_enqueue_scripts', array( $this, 'riven_related_metabox_enqueues' ) );
        add_action( 'add_meta_boxes', array( $this, 'riven_related_add_metabox' ) );
        add_action( 'save_post', array( $this, 'riven_related_metabox_save' ) );
    }

    function riven_related_metabox_enqueues( $screen ) {
    ...
    function riven_related_add_metabox( $post_type ) {
        if ( in_array( $post_type, $this->supported_post_types ) ) {
            add_meta_box(
                'related-metabox',
                esc_html__('Related post', 'riven' ),
    ...
    function riven_related_metabox_save( $post_id ) {
...
if ( is_admin() ) {
    add_action( 'load-post.php', 'rivent_sn_related_init' );
    add_action( 'load-post-new.php', 'rivent_sn_related_init' );
}

function rivent_sn_related_init() {
    new riven_Related_Posts();
}
