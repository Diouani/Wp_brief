...
add_action('init', 'riven_like_scripts');

add_action('wp_ajax_nopriv_riven-post-like', 'riven_post_like');
add_action('wp_ajax_riven-post-like', 'riven_post_like');
function riven_post_like() {
    $nonce = $_POST['nonce'];
    if (!wp_verify_nonce($nonce, 'ajax-nonce'))
        die('Nope!');

    if (isset($_POST['riven_post_like'])) {
        $like_type = 'post';
...
function riven_getPostLikeLink( $post_id ) {
    ...
    $output = '<a href="#" class="riven-post-like'.$class.'" data-post_id="'.$post_id.'" title="'.$title.'">'.$heart.'&nbsp;'.'</a>';
    return $output;
}
?>